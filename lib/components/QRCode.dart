...
class QRCodeForm extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return QRCodeFormState();
  }

}

class QRCodeFormState extends State<QRCodeForm> {
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            FutureBuilder<String>(
              future: _qrcodeString,
              builder: (BuildContext context, AsyncSnapshot<String> snapshot){
                this.QRCodeController.text = snapshot.data != null ? snapshot.data : '';
                return TextFormField(
                  controller: this.QRCodeController,
                  enabled: false
                );
              }
            ),
            RaisedButton(
              ...
              child: Container(
                ...
                child: Center(child: Text('Scanner un QRCode', style: TextStyle(color: Colors.white)))
              ),
              onPressed: (){
                setState(() {
                  _qrcodeString = new QRCodeReader().scan();
                });
              },
            ),
                ...
                if(_formKey.currentState.validate()){
                  ...
                  if(results.length > 0){
                    ...
                    if(checkIfOccupied.length > 0) {
                      ...
                      await conn.query('INSERT INTO belongings(belongings_type, belongings_cloakroom, belongings_number, belongings_location, belongings_info) VALUES(?, ?, ?, ?, ?)', [DescController.text, cloakroomKey, QRCodeController.text, LocationController.text, InfoController.text]);
                      Scaffold.of(context).hideCurrentSnackBar();
